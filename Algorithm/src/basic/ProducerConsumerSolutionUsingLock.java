...
class ProducerConsumerImpl { // producer consumer problem data 
	...
	private final Random theRandom = new Random(); // lock and condition variables 
  ...
  class Producer extends Thread { 
	...
	public Producer(ProducerConsumerImpl sharedObject) { 
		super("PRODUCER"); this.pc = sharedObject;
	}
	@Override 
	public void run() { 
		try { pc.put(); } catch (InterruptedException e) { e.printStackTrace(); 
		} 
	  } 
   } 
...
class Consumer extends Thread { 
	...
	public Consumer(ProducerConsumerImpl sharedObject) {
		super("CONSUMER"); this.pc = sharedObject; 
	}
	@Override 
	public void run() { 
		try { pc.get(); 
